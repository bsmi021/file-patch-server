...
const DEFAULT_WHITESPACE_CONFIG = {
...
class FilePatchServer {
    ...
    isMethodMatch(search, current) {
        // Name must match exactly
        if (search.name !== current.name)
            return false;
        ...
        if (searchParams !== currentParams) {
            ...
            if (searchParamList.length !== currentParamList.length)
                return false;
        ...
        if (search.modifiers && search.modifiers.length > 0) {
            ...
            for (const modifier of search.modifiers) {
                if (!current.modifiers.includes(modifier))
                    return false;
            }
        }
    ...
    async validatePatch(operation) {
        ...
        switch (operation.type) {
            ...
            default:
                errors.push(`Unknown patch type: ${operation.type}`);
        }
        return {
    ...
    normalizeContent(content, config = DEFAULT_WHITESPACE_CONFIG) {
        const lines = content.split(/\r\n|\r|\n/);
    ...
    trackChanges(original, modified) {
    ...
    setupProgressHandling() {
        this.chunkProcessor.on('progress', (progress) => {
