 ...
 * ================
 * 
 * A Model Context Protocol (MCP) server that provides sophisticated file patching capabilities.
 * This server enables precise modifications to files with support for various patching strategies
 * and robust handling of whitespace, indentation, and large files.
 * 
 * Key Features:
 * ------------
 * - Multiple patch operation types (line, block, diff, complete)
 * - Smart code block detection and replacement
 * - Method signature matching for precise updates
 * - Efficient large file handling through chunked processing
 ...
 * --------------------
 * 1. Line Patch: Replace or delete specific lines by number or pattern
 * 2. Block Patch: Replace code blocks while preserving structure and indentation
 * 3. Diff Patch: Apply unified diff format patches
 * 4. Complete Update: Replace entire file content with proper whitespace handling
 * 
 * Usage:
 * -----
 * The server exposes a single tool 'apply_patch' that accepts patch operations with:
 * - type: The type of patch operation to perform
 * - filePath: Path to the file to modify
 * - search: Pattern or content to find (for line/block operations)
 * - replace: Content to replace with
 * - createBackup: Whether to create a backup before patching
 * - whitespaceConfig: Configuration for whitespace handling
 * 
 * Example:
 * -------
 * {
 ...
 *     "search": "function calculate() {",
 *     "replace": "function calculate() { // New implementation",
 *     "createBackup": true
 *   }
 * }
 * 
 * @module file-patch-server
 * @version 1.0.0
 */
        ...
        try {
            ...
            const fileContent = stats.size >= (DEFAULT_BATCH_CONFIG.maxChunkSize || 1024 * 1024)
                ? await (async () => {
                    ...
                    await this.chunkProcessor.processFile(
                        operation.filePath,
